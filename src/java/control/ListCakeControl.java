...
public class ListCakeControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ShareInterface shareDAO = new ShareDAO();
            List<Share> share = shareDAO.getShare();
            request.setAttribute("share", share);
            CakeInterface cakeDAO = new CakeDAO();
            String page_index = request.getParameter("txtPage");
            page_index = page_index != null ? page_index : "1";
            int pageIndex = Integer.parseInt(page_index);

            final int pageSize = 4;
            Pagination pagination = new Pagination(pageIndex, pageSize, cakeDAO.getTotalProducts());

            List<Cake> cakes = cakeDAO.getAllCakes(pagination.getStartIndex(), pagination.getEndIndex());
            System.out.println(cakes.size());
            request.setAttribute("cakes", cakes);
            request.setAttribute("maxPage", pagination.getPaginationSize());
            request.setAttribute("pageIndex", pageIndex);
            ViewInterface viewDAO = new ViewDAO();
            request.setAttribute("view", viewDAO.getView());
